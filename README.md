...
なお、被写体サイズの上限(小: 3%→5%、既定値: 小→中(15%))はもともとブラウザ側の制約に起因する変更ではありませんでしたが、オリジナル版側にも同じ変更が反映されたため、現在は両版で共通の設定です。
...
- オリジナル版には `align.py` の純粋関数(最大内接矩形、ホモグラフィのrescalingなど)とAPIバリデーションを対象にした `pytest` テストスイート(`tests/`)がありますが、Web版のリポジトリには自動テストは含まれていません(開発中に実ブラウザでの動作確認は行っていますが、テストコードとしてはコミットしていません)。
- オリジナル版には各処理段階を実写真の中間結果画像つきで解説する [`docs/pipeline-walkthrough.md`](https://github.com/MeiNogizaka/BurstAlignImage/blob/main/docs/pipeline-walkthrough.md) がありますが、Web版では移植していません。アルゴリズムの解説はオリジナル版のドキュメントを参照してください(処理内容そのものはこのWeb版でも同じです)。
